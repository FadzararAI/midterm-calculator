...
double Calculator::evaluate(string problem){
	...
	for (i=0; i < problem.length(); i++){
		...
		//closed bracket, solve inside the bracket
			...
			while (!ops.characterEnqueue() && ops.getRear() != '('){
				//take first number 
				...
				values.numberEnqueue(operate(val1, val2, op)); 
			}
		
			if (!ops.characterEnqueue()){
				ops.Dequeue();
			}
		}
		...
		//if operator
			...
			while (!ops.characterEnqueue() && precedence(ops.getRear()) >= precedence(problem[i])){
				double val1 = values.getRear();
				values.Dequeue();
